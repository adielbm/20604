...
- ב. משולש בגרף לא מכוון מוגדר כתת-גרף בן 3 צמתים כך שיש קשת בין כל זוג של צמתי המשולש (קליקה בגודל 3). הוכיחו ששפה הבאה שייכת למחלקה P:  $\mathit{TRIANGLE} = \{\langle G \rangle: G \text{ contains at least one triangle}\}$
	- ֹֹ**תשובה:** נראה אלגוריתם פולינומיאלי עבור הבעיה:
		...
		- לגרף עם $n$ צמתים יש לכל היותר $\binom{n}{3} = \frac{n!}{3!(n-3)!} = O(n^3)$ שלשות צמתים. לכן שלב 2 יבוצע לכל היותר $O(n^3)$ פעמים. לכן כל השלבים הם רצים בזמן פולינומיאלי. לכן $\mathit{TRIANGLE}\in \mathrm{P}$.
...
- א. בנו DFA שמזהה את השפה מעל הא"ב הבינארי של המילים המכילות אות "1" לפחות פעם אחת, ומכילות אות "0" לכל היותר פעם אחת.
	- **תשובה:** (קרדיט: [almogfishel](https://github.com/almogfishel))
		- ![](images/2025a-b-q1a.excalidraw.svg)
		- מצב $q_0$ - התחלה, עוד לא קראנו אף תו בפרט לא 1.
		- מצב $q_1$ - ראינו רק 1-ים, קלט תקין.
		- מצב $q_2$ - ראינו 0 יחיד. עדין חסר 1.
		- מצב $q_3$ - או (i) שראינו 1-ים בהתחלה (לפחות פעם אחת) ולאחריו 0 יחיד וייתכנו עוד 1-ים אחרי. קלט תקין. או (ii) שראינו 0 יחיד. ואז לפחות 1 אחד. גם תקין.
		- מצב $\emptyset$ - מלכודת - כבר היו שתי אפסים.
...
- ב. האם השפה המתקבלת משרשור שתי שפות שניתנות לזיהוי גם היא שפה ניתנת לזיהוי?
	- **תשובה:** כן, השפה המתקבלת משרשור שתי שפות שניתנות לזיהוי גם היא שפה ניתנת לזיהוי.
		...
		- נבנה מ"ט לא דטרמיניסטית $M'$ כך: "עבור כל קלט $w$:
			- נחלק באופן לא דטרמיניסטי את $w$ לשני חלקים $x$ ו- $y$, כך ש- $w=xy$.
			- נריץ את $M_1$ על $x$. 
...
- א. האם השפה הבאה מעל הא"ב $\{0,1\}$ רגולרית? אם כן הוכיחו בעזרת בניית DFA.
	...
	- **תשובה:** השפה רגולרית. נבנה DFA: ![](images/img19.excalidraw.svg)
...
- ב. יהי $G=(V,E)$ גרף לא מכוון. קבוצת קודקודים $S$ נקראת כמעט-קליקה אם אפשר להוסיף לכל היותר צלע אחת בין שני קודקודים של $S$ ובכך להפוך אותה $S$ לקליקה. 
	...
	- **תשובה:** 
		...
		- (2) נראה רדוקציה פולי' מ- CLIQUE ל- $L$. כלומר נראה ש- $CLIQUE\leq_{\text{P}} L$
			- נגדיר פונקציה $f:CLIQUE\to L$ כך: $f(\langle G, t \rangle)=\langle G', t+2\rangle$, כאשר $G'=(V',E')$ הוא הגרף המתקבל מ- $G=(V,E)$ על ידי הוספת שני צמתים חדשים $v_{n+1}$ ו- $v_{n+2}$, והוספת צלעות בין כל צומת ב- $V$ לבין $v_{n+1}$ ו- $v_{n+2}$. (אך לא בין $v_{n+1}$ ל- $v_{n+2}$). 
