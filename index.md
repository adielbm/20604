...
- א. הוכיחו או הפריכו: אם $L^*$ שפה רגולרית, אזי גם $L$ שפה רגולרית.
	- **תשובה:** הטענה אינה נכונה. נראה דוגמה נגדית:
		...
		- אבל $L^*$ היא שפה רגולרית, כי היא מכילה את כל המחרוזות של $a$-ים, באורך גדול או שווה ל- 2. כי $L$ מכילה את $aa$ ו- $aaa$. כלומר $L^* = \Sigma^* \setminus \{\varepsilon,a\}$. ושפות רגולריות סגורות תחת הפרש קבוצות.
...
- א. האם השפה הבאה היא שפה חסרת הקשר? הוכיחו או הפריכו. $L=\{ a^n b^m : n \neq m\}$.
	- נוכיח שכן בעזרת שני דקדוקים חסרי הקשר עבור שני שפות:
...
- האם השפה הבאה חסרת הקשר. $L=\{0^i 1^j 2^i 3^j \mid i,j \geq 0\}$.
	- **תשובה:** נניח בשלילה שהשפה חסרת הקשר. 
		- לפי למת הניפוח, המחרוזת $s=0^p1^p 2^p 3^p$, ניתנת לכתיבה כ- $s=uvxyz$, כאשר $|vxy| \leq p$ ו- $|vy|>0$.
		- ננפח למעלה, $uv^2xy^2z$, נקבל מחרוזת שאינה בשפה כי $vxy$ אינה יכולה להכיל גם 0-ים וגם 1-ים וגם 2-ים וגם 3-ים, ולכן לא ניתן לשמור על היחס בניהם כאשר ננפח. 
		- מסקנה: השפה אינה חסרת הקשר.


- הוכיחו או הפריכו: אם $A\leq_P B$ אז יתכן ש- $A$ שייכת ל P אבל $B$ לא שייכת ל P.
...
- הוכיחו בעזרת רדוקציית מיפוי שהשפה המשלימה של השפה הבאה אינה ניתנת לזיהוי ע"י מ"ט. $L=\{ \langle M , D \rangle \mid M \text{ is a TM, } D \text{ is a DFA, and } L(M) = L(D) \}$
	- **תשובה:**
...
- הוכיחו או הפריכו: מכל שפה $A$ אפשר לבנות רדוקציית מיפוי לשפה $A$. כלומר: $A \leq_m A$.
	- **תשובה:** הטענה נכונה. ניתן לבנות את הרדוקציה $f(w)=w$. ואז $w\in A \iff f(w)\in A$. 


- הוכיחו או הפריכו: לכל שפה $A$ מתקיים: $A \leq_m \overline{A}$. 
	- **תשובה:** הטענה אינה נכונה. דוגמה נגדית: 
