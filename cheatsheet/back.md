...
- ${A_{TM}} \leq_{\mathrm{m}} L=\{ \langle \underset{ \textsf{TM} }{ M },\underset{ \textsf{DFA} }{ D }\rangle \mid L(M)=L(D) \};$ $f(\langle M,w\rangle)=\langle M',D\rangle$, where $M'=$"On $x$: if $x=w$ return $M(x)$; O/W, 🅁;" $D$ is DFA s.t. $L(D)=\{w\}.$ 
- $A\leq_{\text{m}} HALT_{\textsf{TM}};$ $f(w)=\langle M,\varepsilon\rangle$, where $M=$"On $x$: if $w\in A$, halt; if $w\notin A$, loop;"  
- $A_{TM}\leq_{\text{m}} CF_{\textsf{TM}}=\{ \langle M \rangle \mid L(M) \text{ is CFL} \};$ $f(\langle M,w\rangle)=\langle N \rangle$, where $N=$"On $x$: if $x=a^n b^nc^n$, 🅐; O/W, return $M(w)$;" 
...
- $\overline{\text{HALT}_{\textsf{TM}}} \leq_{\text{m}} \{\,\langle M_{\textsf{TM}}\rangle : M\text{ 🅐 all even num.}\};\quad$ $f(\langle M, w \rangle) = \langle M' \rangle$, where $M'=$"On $x$: 🅁 if $M(w)$ halts within $|x|$. O/W, 🅐" 
...
- $\text{CLIQUE}$, $\text{SUBSET-SUM}$, $\text{SAT}$, $\text{3SAT}$, $\overset{\text{VERTEX}}{\small\text{COVER}}$, $\text{HAMPATH}$, $\text{UHAMATH}$, $3COLOR\in\text{NP-complete}.\quad$ $\emptyset,\Sigma^*\notin\text{NP-complete}$. 
...
- $3COLOR \leq_{\mathrm{P}} \overset{ \textsf{almost} }{ 3COLOR };$ $f(\langle G \rangle) = \langle G' \rangle,\,G'=G\cup K_4$
...
- $\text{CLIQUE}_{k} \leq_{\mathrm{P}}\overset{\textsf{almost}}{\text{CLIQUE}_{k}};$ $f(\langle G, k \rangle) = \langle G', k+2 \rangle$, where $G'=G\cup \{ v_{n+1},v_{n+2} \}$ and $v_{n+1},v_{n+2}$ are con. to all $G$ nodes.
